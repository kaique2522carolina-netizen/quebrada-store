...
import { ChevronLeft, CheckCircle, Lock, Zap, CreditCard, FileText } from 'lucide-react'
...
export default function CheckoutPage({ onBack, onSuccess, toast }) {
  ...
  const handleConfirm = () => {
    // TODO: call createPaymentPreference() from services/payment.js
    // const preference = await createPaymentPreference({ items, form, total })
  ...
  if (confirmed) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center px-6 text-center gap-6 page-enter">
        ...
        <RippleButton
          className="w-full bg-white text-black font-black py-4 rounded-2xl text-sm tracking-widest"
          onClick={onSuccess}
        >
          CONTINUAR COMPRANDO
        </RippleButton>
  ...
  return (
    <div className="min-h-screen bg-black pt-16 pb-36 page-enter">
      ...
      <div className="flex items-center gap-4 px-4 py-4 border-b border-white/10">
        <button onClick={step > 0 ? () => setStep(s => s - 1) : onBack} className="text-zinc-400 hover:text-white transition-colors">
          <ChevronLeft size={24} />
        </button>
        ...
        {step === 0 && (
          <div className="space-y-4 animate-fade-in">
            ...
            <Field label="Sobrenome *">
              <input className={inputCls} placeholder="Seu sobrenome" value={form.sobrenome} onChange={e => set('sobrenome', e.target.value)} />
            </Field>
            <Field label="CPF *">
              <input className={inputCls} placeholder="000.000.000-00" value={form.cpf} onChange={e => set('cpf', fmtCPF(e.target.value))} inputMode="numeric" />
            </Field>
            <Field label="WhatsApp *">
              <input className={inputCls} placeholder="(11) 99999-9999" value={form.whatsapp} onChange={e => set('whatsapp', fmtPhone(e.target.value))} inputMode="tel" />
            </Field>
          </div>
        )}
          ...
          <div className="space-y-4 animate-fade-in">
            ...
            <Field label="CEP *">
              <div className="relative">
                <input className={inputCls} placeholder="00000-000" value={form.cep} onChange={e => handleCepChange(e.target.value)} inputMode="numeric" />
                {cepLoading && (
            ...
            {address && (
              <>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Field label="Número *">
                    <input className={inputCls} placeholder="123" value={form.numero} onChange={e => set('numero', e.target.value)} inputMode="numeric" />
                  </Field>
                  <Field label="Complemento">
                    <input className={inputCls} placeholder="Apto, Sala..." value={form.complemento} onChange={e => set('complemento', e.target.value)} />
                  </Field>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <Field label="Cidade">
        ...
        {step === 2 && (
          <div className="space-y-5 animate-fade-in">
            ...
            <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-2xl p-4">
              <p className="text-yellow-400 text-xs font-black mb-1">⚡ MERCADO PAGO — PRONTO PARA INTEGRAR</p>
              <p className="text-zinc-500 text-[11px] leading-relaxed">
                Adicione MP_ACCESS_TOKEN e VITE_MP_PUBLIC_KEY no arquivo .env do backend para ativar pagamentos reais.
              </p>
            </div>
          </div>
        )}
      ...
      <div className="fixed bottom-0 left-0 right-0 px-4 pb-8 pt-4 bg-gradient-to-t from-black via-black to-transparent z-50">
        <RippleButton
          ...
          onClick={handleNext}
        >
          {step < STEPS.length - 1 ? `CONTINUAR → ${STEPS[step + 1].toUpperCase()}` : 'CONFIRMAR PEDIDO'}
        </RippleButton>
        <div className="flex items-center justify-center gap-1.5 mt-3">
          <Lock size={11} className="text-zinc-600" />
          <p className="text-zinc-600 text-[10px] font-medium">Pagamento 100% seguro · SSL</p>
        </div>
      </div>
    </div>
  )
...
const inputCls = 'w-full bg-zinc-900 border border-white/10 rounded-xl px-4 py-3.5 text-white text-sm focus:outline-none focus:border-white/30 transition-colors placeholder-zinc-600'
